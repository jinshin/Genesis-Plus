...
static double luminanceWeight            = 1;
static double equalColorTolerance        = 30;
static double dominantDirectionThreshold = 3.6;
static double steepDirectionThreshold    = 2.2;

namespace
{
template <BUint32 N> inline
unsigned char getByte(BUint32 val) { return static_cast<unsigned char>((val >> (8 * N)) & 0xff); }

inline unsigned char getAlpha(BUint32 pix) { return getByte<3>(pix); }
inline unsigned char getRed  (BUint32 pix) { return getByte<2>(pix); }
inline unsigned char getGreen(BUint32 pix) { return getByte<1>(pix); }
inline unsigned char getBlue (BUint32 pix) { return getByte<0>(pix); }

inline BUint32 makePixel(                 unsigned char r, unsigned char g, unsigned char b) { return             (r << 16) | (g << 8) | b; }
inline BUint32 makePixel(unsigned char a, unsigned char r, unsigned char g, unsigned char b) { return (a << 24) | (r << 16) | (g << 8) | b; }


template <unsigned int M, unsigned int N> inline
BUint32 gradientRGB(BUint32 pixFront, BUint32 pixBack) //blend front color with opacity M / N over opaque background: http://en.wikipedia.org/wiki/Alpha_compositing#Alpha_blending
{
...
template <unsigned int M, unsigned int N> inline
BUint32 gradientARGB(BUint32 pixFront, BUint32 pixBack) //find intermediate color between two colors with alpha channels (=> NO alpha blending!!!)
{
    static_assert(0 < M && M < N && N <= 1000, "");
...
BUint32*       byteAdvance(      BUint32* ptr, int bytes) { return reinterpret_cast<      BUint32*>(reinterpret_cast<      char*>(ptr) + bytes); }
const BUint32* byteAdvance(const BUint32* ptr, int bytes) { return reinterpret_cast<const BUint32*>(reinterpret_cast<const char*>(ptr) + bytes); }


//fill block  with the given color
inline
void fillBlock(BUint32* trg, int pitch, BUint32 col, int blockWidth, int blockHeight)
{
...
inline
void fillBlock(BUint32* trg, int pitch, BUint32 col, int n) { fillBlock(trg, pitch, col, n, n); }
...
{
public:
    OutputMatrix(BUint32* out, int outWidth) : //access matrix area, top-left at position "out" for image with given width
        out_(out),
        outWidth_(outWidth) {}
...
inline
double distRGB(BUint32 pix1, BUint32 pix2)
{
...
inline
double distYCbCr(BUint32 pix1, BUint32 pix2, double lumaWeight)
{
...
{
public:
    static double dist(BUint32 pix1, BUint32 pix2)
    {
...
private:
    ...
    {
        for (BUint32 i = 0; i < 256 * 256 * 256; ++i) //startup time: 114 ms on Intel Core i5 (four cores)
        {
    ...
    double distImpl(BUint32 pix1, BUint32 pix2) const
    {
...
BlendResult preProcessCorners(const Kernel_4x4& ker) //result: F, G, J, K corners of "GradientType"
{
    ...
    auto dist = [&](BUint32 pix1, BUint32 pix2) { return ColorDistance::dist(pix1, pix2, luminanceWeight); };
    ...
    {
        const bool dominantGradient = dominantDirectionThreshold * jg < fk;
    ...
    {
        const bool dominantGradient = dominantDirectionThreshold * fk < jg;
...
#define DEF_GETTER(x) template <RotationDegree rotDeg> BUint32 inline get_##x(const Kernel_3x3& ker) { return ker.x; }
...
#define DEF_GETTER(x, y) template <> inline BUint32 get_##x<ROT_90>(const Kernel_3x3& ker) { return ker.y; }
...
#define DEF_GETTER(x, y) template <> inline BUint32 get_##x<ROT_180>(const Kernel_3x3& ker) { return ker.y; }
...
#define DEF_GETTER(x, y) template <> inline BUint32 get_##x<ROT_270>(const Kernel_3x3& ker) { return ker.y; }
...
#define i get_i<rotDeg>(ker)
    ...
    {
        auto eq   = [&](BUint32 pix1, BUint32 pix2) { return ColorDistance::dist(pix1, pix2, luminanceWeight) < equalColorTolerance; };
        auto dist = [&](BUint32 pix1, BUint32 pix2) { return ColorDistance::dist(pix1, pix2, luminanceWeight); };
        ...
        const BUint32 px = dist(e, f) <= dist(e, h) ? f : h; //choose most similar color
        ...
        if (doLineBlend)
        {
            ...
            const bool haveShallowLine = steepDirectionThreshold * fg <= hc && e != g && d != g;
            const bool haveSteepLine   = steepDirectionThreshold * hc <= fg && e != c && b != c;
...
void scaleImage(const BUint32* src, BUint32* trg, int srcWidth, int srcHeight, int yFirst, int yLast)
{
    ...
    //"sizeof(BUint32) * srcWidth * (yLast - yFirst)" bytes without risk of accidental overwriting before accessing
    ...
    if (yFirst > 0)
    {
        ...
        const BUint32* s_m1 = src + srcWidth * std::max(y - 1, 0);
        const BUint32* s_0  = src + srcWidth * y; //center line
        const BUint32* s_p1 = src + srcWidth * std::min(y + 1, srcHeight - 1);
        const BUint32* s_p2 = src + srcWidth * std::min(y + 2, srcHeight - 1);
    ...
    {
        BUint32* out = trg + Scaler::scale * y * trgWidth; //consider MT "striped" access
        ...
        for (int x = 0; x < srcWidth; ++x, out += Scaler::scale)
        {
            ...
            fillBlock(out, trgWidth * sizeof(BUint32), ker4.f, Scaler::scale); //place *after* preprocessing step, to not overwrite the results while processing the the last pixel!
...
struct Scaler2x : public ColorGradient
{
    ...
    static void alphaGrad(BUint32& pixBack, BUint32 pixFront) { ColorGradient::template alphaGrad<M, N>(pixBack, pixFront); }


    template <class OutputMatrix>
    static void blendLineShallow(BUint32 col, OutputMatrix& out)
    {
    ...
    template <class OutputMatrix>
    static void blendLineSteep(BUint32 col, OutputMatrix& out)
    {
    ...
    template <class OutputMatrix>
    static void blendLineSteepAndShallow(BUint32 col, OutputMatrix& out)
    {
    ...
    template <class OutputMatrix>
    static void blendLineDiagonal(BUint32 col, OutputMatrix& out)
    {
        alphaGrad<1, 2>(out.template ref<1, 1>(), col);
    }

    template <class OutputMatrix>
    static void blendCorner(BUint32 col, OutputMatrix& out)
    {
...
{
    static double dist(BUint32 pix1, BUint32 pix2, double luminanceWeight)
    {
...
struct ColorGradientRGB
{
    ...
    static void alphaGrad(BUint32& pixBack, BUint32 pixFront)
    {
...
void xbrz::scale(size_t factor, const BUint32* src, BUint32* trg, int srcWidth, int srcHeight, ColorFormat colFmt, int yFirst, int yLast)
{
    switch (colFmt)
    {
        case ColorFormat::ARGB:
            ...
            {
                case 2:
                    return scaleImage<Scaler2x<ColorGradientARGB>, ColorDistanceARGB>(src, trg, srcWidth, srcHeight, yFirst, yLast);
                case 3:
                    return scaleImage<Scaler3x<ColorGradientARGB>, ColorDistanceARGB>(src, trg, srcWidth, srcHeight, yFirst, yLast);
                case 4:
                    return scaleImage<Scaler4x<ColorGradientARGB>, ColorDistanceARGB>(src, trg, srcWidth, srcHeight, yFirst, yLast);
                case 5:
                    return scaleImage<Scaler5x<ColorGradientARGB>, ColorDistanceARGB>(src, trg, srcWidth, srcHeight, yFirst, yLast);
                case 6:
                    return scaleImage<Scaler6x<ColorGradientARGB>, ColorDistanceARGB>(src, trg, srcWidth, srcHeight, yFirst, yLast);
            }
            break;
        ...
        case ColorFormat::RGB:
            ...
            {
                case 2:
                    return scaleImage<Scaler2x<ColorGradientRGB>, ColorDistanceRGB>(src, trg, srcWidth, srcHeight, yFirst, yLast);
                case 3:
                    return scaleImage<Scaler3x<ColorGradientRGB>, ColorDistanceRGB>(src, trg, srcWidth, srcHeight, yFirst, yLast);
                case 4:                                                                                            
                    return scaleImage<Scaler4x<ColorGradientRGB>, ColorDistanceRGB>(src, trg, srcWidth, srcHeight, yFirst, yLast);
                case 5:
                    return scaleImage<Scaler5x<ColorGradientRGB>, ColorDistanceRGB>(src, trg, srcWidth, srcHeight, yFirst, yLast);
                case 6:
                    return scaleImage<Scaler6x<ColorGradientRGB>, ColorDistanceRGB>(src, trg, srcWidth, srcHeight, yFirst, yLast);
            }
            break;
    }
    assert(false);
...
bool xbrz::equalColorTest(BUint32 col1, BUint32 col2, ColorFormat colFmt, double luminanceWeight, double equalColorTolerance)
{
    switch (colFmt)
    {
        case ColorFormat::ARGB:
...
void xbrz::nearestNeighborScale(const BUint32* src, int srcWidth, int srcHeight, int srcPitch,
                                BUint32* trg, int trgWidth, int trgHeight, int trgPitch,
                                SliceType st, int yFirst, int yLast)
{
    if (srcPitch < srcWidth * static_cast<int>(sizeof(BUint32))  ||
        trgPitch < trgWidth * static_cast<int>(sizeof(BUint32)))
    {
        assert(false);
        return;
    }
    ...
    switch (st)
    {
        case NN_SCALE_SLICE_SOURCE:
            ...
            for (int y = yFirst; y < yLast; ++y)
            {
                ...
                if (blockHeight > 0)
                {
                    const BUint32* srcLine = byteAdvance(src, y * srcPitch);
                    BUint32* trgLine  = byteAdvance(trg, yTrg_first * trgPitch);
                    int xTrg_first = 0;
        ...
        case NN_SCALE_SLICE_TARGET:
            ...
            for (int y = yFirst; y < yLast; ++y)
            {
                BUint32* trgLine = byteAdvance(trg, y * trgPitch);
                const int ySrc = srcHeight * y / trgHeight;
                const BUint32* srcLine = byteAdvance(src, ySrc * srcPitch);
...
extern "C" void xBRZScale_2X (BUint32* input, BUint32* output) {
        scaleImage<Scaler2x<ColorGradientRGB>, ColorDistanceRGB>(input, output, 320, 224, 0, 224);
}
