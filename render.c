...
#ifndef _WFIX
void RenderLine_RL (uint8 *src, int line, int offset, uint16 *table, int length);
void RenderLine_RR (uint8 *src, int line, int offset, uint16 *table, int length);
void RenderLine_P  (uint8 *src, int line, int offset, uint16 *table, int length);
void RenderLine_P_Crop  (uint8 *src, int line, int offset, uint16 *table, int length);
void RenderLine_RL_F (uint8 *src, int line, int offset, uint16 *table, int length);
void RenderLine_RR_F (uint8 *src, int line, int offset, uint16 *table, int length);
void RenderLine_P_F  (uint8 *src, int line, int offset, uint16 *table, int length);
void RenderLine_P_Crop_F  (uint8 *src, int line, int offset, uint16 *table, int length);
#else 
...
extern FILE *mylog;
if ((!fast_vid)|(NeedMessage)) {
    if (rotateright) { RenderLine = RenderLine_RR; } else { RenderLine = RenderLine_RL; };
    if (!landscape) { 
		if (!crop_screen) RenderLine = RenderLine_P; else if (crop_screen==2) {
									if (vdp.reg[12] & 1) RenderLine = RenderLine_P; else RenderLine = RenderLine_P_Crop;
									}
									else RenderLine = RenderLine_P_Crop;
		}
    if (square_screen) {
      		if (!crop_screen) RenderLine = RenderLine_P; else if (crop_screen==2) {
									if (vdp.reg[12] & 1) RenderLine = RenderLine_P; else RenderLine = RenderLine_P_Crop;
...
//fprintf(stderr,("Using cached render\n"));

UncachedRender=0;

} else
{
    if (rotateright) { RenderLine = RenderLine_RR_F; } else { RenderLine = RenderLine_RL_F; };
    if (!landscape) { 
		if (!crop_screen) RenderLine = RenderLine_P_F; else if (crop_screen==2) {
									if (vdp.reg[12] & 1) RenderLine = RenderLine_P_F; else RenderLine = RenderLine_P_Crop_F;
									}
									else RenderLine = RenderLine_P_Crop_F;
		}
    if (square_screen) {
      		if (!crop_screen) RenderLine = RenderLine_P_F; else if (crop_screen==2) {
									if (vdp.reg[12] & 1) RenderLine = RenderLine_P_F; else RenderLine = RenderLine_P_Crop_F;
...
//fprintf(stderr,("Using uncached render\n"));
...
tmp = (uint8*)bitmap.data + ((MUL240(319-offset)+line)<<1);
outWrite = (uint16*)tmp;
tmp = (uint8*)ScreenCache + ((MUL320(line)+offset)<<1);
outCache = (uint16*)tmp;
	do {
		uint16 val = table[*src++];
		if (val!=*outCache) {
			*outWrite = val;
			*outCache = val;
			}
...
uint16 * outCache;
uint8 * tmp;
tmp = (uint8*)bitmap.data + ((MUL240(offset)-line+239)<<1);
outWrite = (uint16*)tmp;
tmp = (uint8*)ScreenCache + ((MUL320(line)+offset)<<1);
outCache = (uint16*)tmp;
	do {
		...
		if (val!=*outCache) {
                        *outWrite = val;
			*outCache = val;
			//*outWrite = ((val&0xF7DE)>>1)+((*outCache&0xF7DE)>>1);
			//*outCache = ((val&0xF7DE)>>1)+((*outCache&0xF7DE)>>1);
			}
		outWrite+=OUTPIXELPITCH;
		outCache++;
	} while (--length);
...
#define RB(x) ((x&RB_MASK)<<8)
#define G(x)  ((x&G_MASK)<<3)

#define P25(x) (x>>2)
#define P50(x) (x>>1)
#define P75(x) ((x>>1)+(x>>2))

#define MAKEPIXEL(rb,g) (((rb>>8)&RB_MASK | (g>>3)&G_MASK))

{
uint16 * outWrite;
uint16 * outCache;
uint8 * tmp;
tmp = (uint8*)bitmap.data + ((MUL240(line)+P75(offset))<<1);
outWrite = (uint16*)tmp;
tmp = (uint8*)ScreenCache + ((MUL240(line)+P75(offset))<<1);
outCache = (uint16*)tmp;
length = length>>2; //We'll process 4 pixels at one shot
	do {
		uint16 p1 = table[*src++];
		uint16 p2 = table[*src++];
		uint16 p3 = table[*src++];
		uint16 p4 = table[*src++];
		uint16 val = MAKEPIXEL(P75(RB(p1))+P25(RB(p2)),P75(G(p1))+P25(G(p2)));
		if (val!=*outCache) {
			*outWrite = val;
			*outCache = val;
			}
		outWrite++;
		outCache++;
		val = MAKEPIXEL(P50(RB(p2))+P50(RB(p3)),P50(G(p2))+P50(G(p3)));
		if (val!=*outCache) {
			*outWrite = val;
			*outCache = val;
			}
		outWrite++;
		outCache++;
		val = MAKEPIXEL(P25(RB(p3))+P75(RB(p4)),P25(G(p3))+P75(G(p4)));
		if (val!=*outCache) {
			*outWrite = val;
			*outCache = val;
			}
...
uint16 * outCache;
uint8 * tmp;
if (length==320) src+=40; else src+=8;
tmp = (uint8*)bitmap.data + ((MUL240(line))<<1);
outWrite = (uint16*)tmp;
tmp = (uint8*)ScreenCache + ((MUL240(line))<<1);
outCache = (uint16*)tmp;
length = 240; 
	do {
		uint32 val = table[*src++];
		if (val!=*outCache) {
			*outWrite = val;
			*outCache = val;
			}
...
uint32 Buffer[640*240];
uint32 Cache[640*240];
...
#define PRE_X2_32_BUF  \
	src[length]=src[length-1]; \
	uint8 *tmp=(uint8*)&Buffer[0] + ((offset+line*320)<<2); \
...
#define PRE_X2_32  \
	src[length]=src[length-1]; \
	uint8 *tmp=(uint8*)bitmap.data + ((offset+line*640)<<3); \
	uint8 *tmp2=(uint8*)&Cache[0] + ((offset+line*320)<<2);	\
	uint32 *outWrite = (uint32*)tmp; \
	uint32 *inCache = (uint32*)tmp2;	
